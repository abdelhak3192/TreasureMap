...
namespace TreasureMap.Objects
{
    internal class SimpleAdvenceMovement : Movement, IMovement
    {
        private readonly IDictionary<Direction, Func<(int, int), (int, int)>> _movements = new Dictionary<Direction, Func<(int, int), (int, int)>>()
            {
                { Direction.North, (position)=> { return (position.Item1-1,position.Item2); } },
                { Direction.South, (position)=> { return (position.Item1+1,position.Item2); } },
                { Direction.East, (position)=> { return (position.Item1,position.Item2+1); } },
                { Direction.West, (position)=> { return (position.Item1,position.Item2-1); } },
            };
